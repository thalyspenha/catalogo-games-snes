...
**Capas locais (offline) — em andamento (2026-07-31).** Bug real achado pelo usuário: capas de jogos somem quando o aparelho fica offline, mesmo já tendo aparecido antes. Causa raiz investigada e confirmada com evidência no S25 (não só leitura de código): o ScreenScraper manda as capas com `Cache-Control: no-cache, must-revalidate`, obrigando o Coil a confirmar com o servidor antes de reusar o arquivo já em disco — sem rede, essa revalidação falha e a imagem não renderiza (falha silenciosa, sem exceção no logcat), mesmo com o cache do Coil populado com os bytes certos. Plano em `docs/superpowers/plans/2026-07-31-capas-locais-offline.md` (4 tasks), sendo executado via subagent-driven-development (ledger em `.superpowers/sdd/2026-07-31-capas-locais-offline/progress.md`, git-ignorado). Solução: baixar a capa durante a sincronização e salvar como arquivo próprio do app (`filesDir/capas/<id>.jpg`), em vez de depender só do cache HTTP do Coil. Task 1/4 concluída: `JogoEntity` ganhou `caminhoCapaLocal: String?` (nullable, default null), banco Room subiu pra versão 3 com migração real (`Migration(2,3)`, `ALTER TABLE jogos ADD COLUMN caminhoCapaLocal TEXT` — sem apagar dado existente). Commit `95815db`, review de task limpo. Tasks 2 (`CapaDownloader` + wiring no `SincronizacaoRepository`), 3 (UI usa capa local com fallback pra URL remota) e 4 (verificação real no S25, replicando o teste do usuário de desligar internet) pendentes.
