...
**Sync em batch do catálogo — em implementação (2026-07-30).** Spec em `docs/superpowers/specs/2026-07-30-sync-batch-screenscraper-design.md`, plano em `docs/superpowers/plans/2026-07-30-sync-batch-screenscraper.md` (12 tasks), sendo executado via subagent-driven-development num worktree isolado (`.claude/worktrees/sync-batch-screenscraper`, branch `worktree-sync-batch-screenscraper`), ledger em `.superpowers/sdd/2026-07-30-sync-batch-screenscraper/progress.md` dentro do worktree. Concluído até aqui: Task 1 (permissão de INTERNET no manifest — faltava, corrigido), Task 2 (infra de teste JUnit), Task 3 (módulo `:ferramentas`, parser/agrupador do DAT No-Intro, testado), Task 4 (`app/src/main/assets/snes_catalogo_mestre.json` gerado — **1763 jogos únicos**, contagem confirmada de forma independente), Task 5 (`SincronizacaoStatusEntity`/`SincronizacaoStatusDao`/`StatusSincronizacao` + `limparTudo()` em `JogoDao`/`PosseUsuarioDao`), Task 6 (`AppDatabase` sobe pra version 2, com `Migration(1,2)` criando a tabela `sincronizacao_status`), Task 7 (`CatalogoMestreItemDto` + `CatalogoMestreLoader`, lê `assets/snes_catalogo_mestre.json`), Task 8 (`calcularRestante()`, função pura testada), Task 9 (`SincronizacaoRepository` — orquestra throttle/retry/cota/cancelamento; revisão em Opus achou 7 Importantes, todos corrigidos numa rodada de fix; ganhou um estado `SincronizacaoEstado.Erro` não previsto no plano original, pra quando faltam credenciais configuradas). Task 10 (`SincronizacaoViewModel`, pass-through simples), Task 11 (`TelaSincronizacao`, Compose). Falta só a Task 12 (navegação — ligar a `CatalogoNavHost`/`TelaBiblioteca` — última do plano). Ao retomar, checar o ledger antes de redespachar qualquer task.
